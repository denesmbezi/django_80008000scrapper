...
def jobs_list_view(request):
    """Display only jobs with valid job_title and location"""
    all_jobs = JobListing.objects.all().order_by('-created_at')

    def is_valid_field(field):
        """Check if a field is valid (not all special chars, contains at least letters)"""
        if not field:
            return False
        field = field.strip()
        return bool(re.search(r'[a-zA-Z]', field)) and not re.match(r'^[*/\-\\|@#\s]+$', field)

    clean_jobs = [
        job for job in all_jobs
        if is_valid_field(job.job_title) and is_valid_field(job.location)
    ]

    return render(request, 'jobs/jobs_list.html', {'jobs': clean_jobs})
